...
It turned-out is not easy to implement and it took solid two weeks to get satisfactory result. In JavaScript you can't get the visible text position from text area and to over-come this limitation I decided to build in real-time shadow HTML tree with elements for each source line and finally those elements position I can map to preview elements. In fact I got smoother scrolling experience then in VS Code or Azure DevOps which I'm really proud.
